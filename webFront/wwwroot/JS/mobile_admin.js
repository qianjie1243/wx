...
var $frame = {
    ...
    //message：提示内容
    //title：提示标题
    //theme  false,true是否启动圆形
    alert: function (message, title, theme) {
        ...
        if (!$frame.IsEmpty(title))
            o['title'] = title;
        if (!$frame.IsEmpty(theme) && theme)
            o['theme'] = 'round-button';

        vant.Dialog.alert(o).then(() => {
            // on close
        });
    ...
    loading: function (msg) {
        ...
        vant.Toast.loading(o);
    },

    //提示详情
    // type 类型success  fail other
    //msg 内容
    alert_msg: function (msg, type, loadingType, position, icon) {
        type = type || 'other';
        msg = msg || "提示内容";
        if (type == 'success')
            ...
            if (!$frame.IsEmpty(loadingType))
                o['loadingType'] = loadingType;
            if (!$frame.IsEmpty(position))
                o['position'] = position;
            if (!$frame.IsEmpty(icon))
                o['icon'] = icon;
            vant.Toast.loading(o);
        }
    },
