...
layui.define(["layer", "laytpl", "upload"], function(i) {
	...
	c.prototype.config = function(i) {
		...
		if (layui.each(Array(5), function(i) {
				...
				isgroup: !0,
				voice: "default.mp3"
			}, i), window.JSON && window.JSON.parse) return j(i), this
		...
		if (window.JSON && window.JSON.parse) return T(i), this
	...
	var u = function(i) {
			...
			return i = i || {}, i.item = i.item || "d." + i.type, ["{{# var length = 0; layui.each(" + i.item +
				", function(i, data){ length++; }}", '<li layim-event="chat" data-type="' + i.type + '" data-index="{{ ' + (i.index ||
					"i") + ' }}" class="layim-' + ("history" === i.type ? "{{i}}" : i.type + "{{data.id}}") +
				' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>',
		...
		y = ['<div class="layui-layim-main">', '<div class="layui-layim-info">',
			...
			"</ul>", "</div>",
			'<input class="layui-layim-remark" placeholder="编辑签名" value="{{ d.mine.remark||d.mine.sign||"" }}">', "</div>",
			'<ul class="layui-unselect layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}', " layim-tab-two",
			'{{# } }}">', '<li class="layui-icon', "{{# if(!d.base.isfriend){ }}", " layim-hide", "{{# } else { }}",
			" layim-this", "{{# } }}", '" title="联系人" layim-event="tab" lay-type="friend">&#xe612;</li>',
			'<li class="layui-icon', "{{# if(!d.base.isgroup){ }}", " layim-hide", "{{# } else if(!d.base.isfriend) { }}",
			" layim-this", "{{# } }}", '" title="群组" layim-event="tab" lay-type="group">&#xe613;</li>',
			'<li class="layui-icon" title="历史会话" layim-event="tab" lay-type="history">&#xe611;</li>', "</ul>",
			'<ul class="layui-unselect layim-tab-content {{# if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">',
			'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}', "<li>",
			...
			'<ul class="layui-layim-list {{# if(spread === "true"){ }}', " layui-show", '{{# } }}">', u({
				type: "friend",
				item: "item.list",
				index: "index"
			}), "</ul>", "</li>", "{{# }); if(d.friend.length === 0){ }}",
			'<li><ul class="layui-layim-list layui-show"><li class="layim-null">暂无联系人</li></ul>', "{{# } }}", "</ul>",
			'<ul class="layui-unselect layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }}">',
			"<li>", '<ul class="layui-layim-list layui-show layim-list-group">', u({
				type: "group"
			}), "</ul>", "</li>", "</ul>",
			'<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">',
			"<li>", '<ul class="layui-layim-list layui-show layim-list-history">', u({
				type: "history"
			}), "</ul>", "</li>", "</ul>", '<ul class="layui-unselect layim-tab-content">', "<li>",
			'<ul class="layui-layim-list layui-show" id="layui-layim-search"></ul>', "</li>", "</ul>",
			...
			"{{# if(!d.base.copyright){ }}",
			'<li class="layui-icon layim-tool-about" layim-event="about" title="关于">&#xe60b;</li>', "{{# } }}", "</ul>",
			'<div class="layui-layim-search"><input><label class="layui-icon" layim-event="closeSearch">&#x1007;</label></div>',
			"</div>"
		].join(""),
		m = ['<ul class="layui-layim-skin">', "{{# layui.each(d.skin, function(index, item){ }}",
			'<li><img layim-event="setSkin" src="{{ item }}"></li>', "{{# }); }}",
			'<li layim-event="setSkin"><cite>简约</cite></li>', "</ul>"
		].join(""),
		f = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">',
			...
			'<div class="layim-chat-send">', "{{# if(!d.base.brief){ }}",
		...
		p = ['<div class="layim-add-box">',
			'<div class="layim-add-img"><img class="layui-circle" src="{{ d.data.avatar }}"><p>{{ d.data.name||"" }}</p></div>',
			'<div class="layim-add-remark">', '{{# if(d.data.type === "friend" && d.type === "setGroup"){ }}', "<p>选择分组</p>",
			'{{# } if(d.data.type === "friend"){ }}', '<select class="layui-select" id="LAY_layimGroup">',
			"{{# layui.each(d.data.group, function(index, item){ }}",
			'<option value="{{ item.id }}">{{ item.groupname }}</option>', "{{# }); }}", "</select>", "{{# } }}",
			'{{# if(d.data.type === "group"){ }}', "<p>请输入验证信息</p>", '{{# } if(d.type !== "setGroup"){ }}',
			'<textarea id="LAY_layimRemark" placeholder="验证信息" class="layui-textarea"></textarea>', "{{# } }}", "</div>",
			"</div>"
		].join(""),
		h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>',
			'<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}",
			'<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}",
			'{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>",
			'<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"
		].join(""),
		v =
		'<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
		g = function(i) {
			return i < 10 ? "0" + (0 | i) : i
		};
	layui.data.date = function(i) {
		var a = new Date(i || new Date);
		return a.getFullYear() + "-" + g(a.getMonth() + 1) + "-" + g(a.getDate()) + " " + g(a.getHours()) + ":" + g(a.getMinutes()) +
			":" + g(a.getSeconds())
		...
		return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(
			/'/g, "&#39;").replace(/"/g, "&quot;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(
			/face\[([^\s\[\]]+?)\]/g,
			function(i) {
			...
			var a = (i.match(/file\(([\s\S]+?)\)\[/) || [])[1],
				e = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
			return a ? '<a class="layui-layim-file" href="' + a +
				'" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (e || a) + "</cite></a>" : i
		}).replace(/audio\[([^\s]+?)\]/g, function(i) {
			return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + i.replace(
				/(^audio\[)|(\]$)/g, "") + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>'
		}).replace(/video\[([^\s]+?)\]/g, function(i) {
			return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" data-src="' + i.replace(
				/(^video\[)|(\]$)/g, "") + '"><i class="layui-icon">&#xe652;</i></div>'
	...
	var x, b, w, k, C, S = function(i, a, n) {
		...
		j = function(i) {
			...
			return mine = a.mine || {}, local = layui.data("layim")[mine.id] || {}, obj = {
				...
				L = e.extend(L, s), H(n(y).render(s)), (l.close || i.min) && I(), layui.each(r.ready, function(i, a) {
					a && a(s)
				})
			}, L = e.extend(L, obj), i.brief ? layui.each(r.ready, function(i, a) {
				a && a(obj)
		...
		H = function(i) {
			return t.open({
				...
				content: i,
				success: function(i) {
					x = i, _(i), L.base.right && i.css("margin-left", "-" + L.base.right), b && t.close(b.attr("times"));
					var a = [],
				...
				cancel: function(i) {
					I();
					var a = layui.data("layim")[L.mine.id] || {};
		...
		I = function(i) {
			return b && t.close(b.attr("times")), x && x.hide(), L.mine = L.mine || {}, t.open({
				type: 1,
				title: !1,
				...
				offset: "rb",
				resize: !1,
				content: '<img src="' + (L.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i ||
					L.base.title || "我的LayIM") + "</span>",
				move: "#layui-layim-close img",
				success: function(i, a) {
					b = i, L.base.right && i.css("margin-left", "-" + L.base.right), i.on("click", function() {
						t.close(a), x.show();
						var i = layui.data("layim")[L.mine.id] || {};
		...
		T = function(i) {
			...
			if (a[0]) {
				...
				return "none" === w.css("display") && w.show(), k && t.close(k.attr("times")), 1 !== s.find("li").length || o[0] ||
					(d || w.css("width", 800), s.css({
						height: w.height()
					}).show(), c.css("margin-left", "200px")), o[0] || (s.append(n(v).render(l)), c.append(n(f).render(l)), z(i), E()),
					O(s.find(".layim-chatlist-" + i.type + i.id)), o[0] || U(), $(i), Q(), C
			}
			l.first = !0;
			var u = C = t.open({
				type: 1,
				...
				anim: i.anim || 0,
				closeBtn: !L.base.brief && 1,
				content: n('<ul class="layui-unselect layim-chat-list">' + v + '</ul><div class="layim-chat-box">' + f +
					"</div>").render(l),
				success: function(a) {
					w = a, a.css({
						"min-width": "500px",
						"min-height": "420px"
					}), z(i), "function" == typeof i.success && i.success(a), Q(), _(a), $(i), U(), q(), layui.each(r.chatChange,
						function(i, a) {
							a && a(R())
		...
		M = function(i) {
			...
			w && !i && w.hide(), t.close(M.index), M.index = t.open({
				type: 1,
				title: !1,
				...
				success: function(a, l) {
					i || (k = a), n.minRight && t.style(l, {
						...
						t.close(l), i ? layui.each(L.chat, function(i, a) {
							T(a)
						}) : w.show(), i && (L.chat = [], Z())
		...
		N = function(i, a) {
			return i = i || {}, t.close(N.index), N.index = t.open({
				type: 1,
				area: "430px",
				title: {
					friend: "添加好友",
		...
		O = function(i, a) {
			...
			if (a) {
				...
				return 1 === u ? t.close(C) : (i.remove(), d.remove(), 2 === u && (w.find(".layim-chat-list").hide(), c || w.css(
					"width", "600px"), w.find(".layim-chat-box").css("margin-left", 0)), !1)
			}
			i.addClass(o).siblings().removeClass(o), d.addClass(s).siblings(l).removeClass(s), d.find("textarea").focus(),
		...
		q = function() {
			var i = R(),
				a = L.message[i.data.type + i.data.id];
			a && delete L.message[i.data.type + i.data.id]
		},
		R = c.prototype.thisChat = function() {
			if (w) {
		...
		_ = function(i) {
			...
			i.css({
				"background-image": e ? "url(" + e + ")" : function() {
					return L.base.initSkin ? "url(" + (layui.cache.dir + "css/modules/layim/skin/" + L.base.initSkin) + ")" :
						"none"
				}()
			})
		},
		...
		D = function() {
			var i = {
					username: L.mine ? L.mine.username : "访客",
					avatar: L.mine ? L.mine.avatar : layui.cache.dir + "css/pc/layim/skin/logo.jpg",
					id: L.mine ? L.mine.id : null,
					mine: !0
				},
				a = R(),
				e = a.elem.find(".layim-chat-main ul"),
				l = L.base.maxLength || 3e3;
		...
		F = function() {
			if (!(l.ie && l.ie < 9)) {
				...
				i.src = layui.cache.dir + "css/modules/layim/voice/" + L.base.voice, i.play()
			}
		},
		G = {},
		K = function(i) {
			...
			var a = e(".layim-chatlist-" + i.type + i.id),
				t = {},
				l = a.index();
			if (i.timestamp = i.timestamp || (new Date).getTime(), i.fromid == L.mine.id && (i.mine = !0), i.system || B(i), G =
				JSON.parse(JSON.stringify(i)), L.base.voice && F(), !w && i.content || l === -1) {
				if (L.message[i.type + i.id]) L.message[i.type + i.id].push(i);
				else if (L.message[i.type + i.id] = [i], "friend" === i.type) {
					var s;
					layui.each(L.friend, function(a, e) {
						if (layui.each(e.list, function(a, e) {
								if (e.id == i.id) return e.type = "friend", e.name = e.username, L.chat.push(e), s = !0
							}), s) return !0
					}), s || (i.name = i.username, i.temporary = !0, L.chat.push(i))
				} else if ("group" === i.type) {
					var o;
					layui.each(L.group, function(a, e) {
						if (e.id == i.id) return e.type = "group", e.name = e.groupname, L.chat.push(e), o = !0
					}), o || (i.name = i.groupname, L.chat.push(i))
				} else i.name = i.name || i.username || i.groupname, L.chat.push(i);
				if ("group" === i.type && layui.each(L.group, function(a, e) {
						if (e.id == i.id) return t.avatar = e.avatar, !0
					}), !i.system) return L.base.notice && J({
			...
			if (w) {
				...
				i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content
					.replace(/\s/g, "") && c.append(n(h).render(i)), Z()
			}
		},
		...
		B = function(i) {
			...
			if (e) {
				...
				}), t || i.fromid == L.mine.id || e.push(i), e.length > d && e.shift()
		...
		P = function(i) {
			...
			if (L[i.type])
				if ("friend" === i.type) layui.each(L.friend, function(n, l) {
					if (i.groupid == l.id) return layui.each(L.friend[n].list, function(e, t) {
						if (t.id == i.id) return a = !0
					}), a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
						anim: 6
					}) : (L.friend[n].list = L.friend[n].list || [], e[L.friend[n].list.length] = i, i.groupIndex = n, L.friend[
						n].list.push(i), !0)
				});
				else if ("group" === i.type) {
				if (layui.each(L.group, function(e, t) {
						if (t.id == i.id) return a = !0
					}), a) return t.msg("您已是 [" + (i.groupname || "") + "] 的群成员", {
					anim: 6
				});
				e[L.group.length] = i, L.group.push(i)
			}
			if (!a) {
				var s = n(u({
					type: i.type,
				...
				if ("friend" === i.type) {
					...
					o.find(".layui-layim-list").append(s), o.find(".layim-count").html(L.friend[i.groupIndex].list.length), o.find(
						".layim-null")[0] && o.find(".layim-null").remove()
				} else "group" === i.type && (l.append(s), l.find(".layim-null")[0] && l.find(".layim-null").remove())
			}
		},
		W = function(i) {
			var a = x.find(".layim-list-" + i.type);
			L[i.type] && ("friend" === i.type ? layui.each(L.friend, function(e, t) {
				layui.each(t.list, function(t, n) {
					if (i.id == n.id) {
						var l = a.find(">li").eq(e);
						l.find(".layui-layim-list>li");
						return l.find(".layui-layim-list>li").eq(t).remove(), L.friend[e].list.splice(t, 1), l.find(".layim-count")
							.html(L.friend[e].list.length), 0 === L.friend[e].list.length && l.find(".layui-layim-list").html(
								'<li class="layim-null">该分组下已无好友了</li>'), !0
					}
				})
			}) : "group" === i.type && layui.each(L.group, function(e, t) {
				if (i.id == t.id) return a.find(">li").eq(e).remove(), L.group.splice(e, 1), 0 === L.group.length && a.html(
					'<li class="layim-null">暂无群组</li>'), !0
			}))
		},
		Z = function() {
			var i = R(),
			...
			if (t >= d) {
				...
				e.prev().hasClass("layim-chat-system") || e.before(
					'<div class="layim-chat-system"><span layim-event="chatLog">查看更多记录</span></div>'), t > d && n.remove()
			}
		...
		Q = function() {
			...
			a.focus(), a.off("keydown").on("keydown", function(i) {
				var e = layui.data("layim")[L.mine.id] || {},
					t = i.keyCode;
		...
		X = function() {
			var i = ["[微笑]", "[嘻嘻]", "[哈哈]", "[可爱]", "[可怜]", "[挖鼻]", "[吃惊]", "[害羞]", "[挤眼]", "[闭嘴]", "[鄙视]", "[爱你]", "[泪]",
		...
		ai = function(i, a) {
			var e, t = i.value;
			i.focus(), document.selection ? (e = document.selection.createRange(), document.selection.empty(), e.text = a) : (
				e = [t.substring(0, i.selectionStart), a, t.substr(i.selectionEnd)], i.focus(), i.value = e.join(""))
		},
		ei = "layui-anim-upbit",
		ti = {
			status: function(i, a) {
				...
				if ("show" === n) ii(a), i.next().show().addClass(ei), e(document).off("click", t).on("click", t);
					...
					var l = i.parent().prev();
					i.addClass(o).siblings().removeClass(o), l.html(i.find("cite").html()), l.removeClass("layim-status-" + (
						"online" === n ? "hide" : "online")).addClass("layim-status-" + n), layui.each(r.online, function(i, a) {
						a && a(n)
					})
				}
			},
			...
			tab: function(i) {
				...
				"number" == typeof i ? (a = i, i = t.eq(a)) : a = i.index(), a > 2 ? t.removeClass(o) : (ti.tab.index = a, i.addClass(
					o).siblings().removeClass(o)), x.find(e).eq(a).addClass(s).siblings(e).removeClass(s)
			},
			spread: function(i) {
				var a = i.attr("lay-type"),
					e = "true" === a ? "false" : "true",
					t = layui.data("layim")[L.mine.id] || {};
				...
				var a = x.find(".layui-layim-search"),
					...
					n = function(i) {
						...
						if ("" === a) ti.tab(0 | ti.tab.index);
							...
							for (var n = [], l = L.friend || [], s = L.group || [], o = "", d = 0; d < l.length; d++)
								for (var r = 0; r < (l[d].list || []).length; r++) l[d].list[r].username.indexOf(a) !== -1 && (l[d].list[r].type =
									"friend", l[d].list[r].index = d, l[d].list[r].list = r, n.push(l[d].list[r]));
							for (var c = 0; c < s.length; c++) s[c].groupname.indexOf(a) !== -1 && (s[c].type = "group", s[c].index = c,
								s[c].list = c, n.push(s[c]));
							if (n.length > 0)
								for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' +
									n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[
										u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
							else o = '<li class="layim-null">无搜索结果</li>';
							e.html(o), ti.tab(3)
						}
					};
				!L.base.isfriend && L.base.isgroup ? ti.tab.index = 1 : L.base.isfriend || L.base.isgroup || (ti.tab.index = 2),
					a.show(), t.focus(), t.off("keyup", n).on("keyup", n)
			},
			...
			chat: function(i) {
				var a = layui.data("layim")[L.mine.id] || {},
					...
					l = {};
				"friend" === e ? l = L[e][t].list[n] : "group" === e ? l = L[e][n] : "history" === e && (l = (a.history || {})[t] ||
					{}), l.name = l.name || l.username || l.groupname, "history" !== e && (l.type = e), T(l)
			},
			tabChat: function(i) {
				O(i)
			},
			...
			groupMembers: function(i, a) {
				...
				i.data("down") ? l() : (n.html("&#xe619;"), i.data("down", !0), ti.groupMembers.index = t.tips(
					'<ul class="layim-members-list"></ul>', i, {
						tips: 3,
						time: 0,
						anim: 5,
						fixed: !0,
						...
						success: function(a) {
							...
							c && s.css({
								...
								layui.each(a.list, function(i, a) {
									o += '<li data-uid="' + a.id + '"><a href="javascript:;"><img src="' + a.avatar + '"><cite>' + a.username +
										"</cite></a></li>", d[a.id] = a
								}), s.html(o), i.find(".layim-chat-members").html(a.members || (a.list || []).length + "人"), s.find(
					...
					}), e(document).off("mousedown", l).on("mousedown", l), e(window).off("resize", l).on("resize", l), i.off(
					"mousedown", s).on("mousedown", s))
			},
			send: function() {
				D()
			},
			setSend: function(i, a) {
				...
				if ("show" === n) ii(a), t.show().addClass(ei), e(document).off("click", ti.setSendHide).on("click", ti.setSendHide);
					...
					var l = layui.data("layim")[L.mine.id] || {};
			...
			image: function(i) {
				...
				layui.upload.render({
					...
					accept: a,
						...
						0 == i.code ? (i.data = i.data || {}, "images" === a ? ai(n.textarea[0], "img[" + (i.data.src || "") + "]") :
							"file" === a && ai(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"), D()
						) : t.msg(i.msg || "上传失败")
					}
				})
			},
			...
			playAudio: function(i) {
				...
				return i.data("error") ? t.msg("播放音频源异常") : e.play ? void(i.attr("status") ? n() : (a || (e.src = i.data("src")),
					e.play(), i.attr("status", "pause"), i.data("audio", e), i.find("i").html("&#xe651;"), e.onended = function() {
						n()
					}, e.onerror = function() {
			...
			playVideo: function(i) {
				...
				return e.play ? (t.close(ti.playVideo.index), void(ti.playVideo.index = t.open({
					...
					maxmin: !0,
					shade: !1,
					content: '<div style="background-color: #000; height: 100%;"><video style="position: absolute; width: 100%; height: 100%;" src="' +
						a + '" loop="loop" autoplay="autoplay"></video></div>'
				}))) : t.msg("您的浏览器不支持video")
			},
			chatLog: function(i) {
				var a = R();
				return L.base.chatLog ? (t.close(ti.chatLog.index), ti.chatLog.index = t.open({
					type: 2,
					maxmin: !0,
					...
					content: L.base.chatLog + "?id=" + a.data.id + "&type=" + a.data.type
				})) : t.msg("未开启更多聊天记录")
			},
			menuHistory: function(i, a) {
				var n = layui.data("layim")[L.mine.id] || {},
					l = i.parent(),
					s = i.data("type"),
