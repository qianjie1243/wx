...
namespace Common
{
    public static class Utils
    {
        ...
        /// 上传文件
        /// </summary>
        /// <param name="ftpPath">PTF文件路径</param>
        /// <param name="hostname">IP地址</param>
        /// <param name="localFile">需要上传的文件路径</param>
        /// <param name="username">FTP账号</param>
        /// <param name="password">FTP密码</param>
        /// <param name="fliename">FTP文件名称</param>
        /// <returns></returns>
        public static Boolean FtpUpload(string ftpPath, string hostname, string localFile, string username, string password, string fliename)
        {

            string URI = "FTP://" + hostname + "/" + ftpPath;
            //检查目录是否存在，不存在创建  
            CheckDirectoryAndMakeMyWilson3(ftpPath);
            //FtpCheckDirectoryExist(ftpPath, URI, username, password);
            FileInfo fi = new FileInfo(localFile);
            FileStream fs = fi.OpenRead();
            long length = fs.Length;
            FtpWebRequest req = (FtpWebRequest)WebRequest.Create(URI + "/" + fliename);
        ...
        //判断文件的目录是否存,不存则创建  
        public static void FtpCheckDirectoryExist(string destFilePath, string URI, string username, string password)
        {
            ...
            for (int i = 0; i < dirs.Length; i++)
            {
                ...
                if (dir != null && dir.Length > 0)
                {
                    ...
                    {
                        curDir += dir + "/";
                        FtpMakeDir(curDir, URI, username, password);
                    }
                    catch (Exception)
                    { }
                }
        ...
        //创建目录  
        public static Boolean FtpMakeDir(string localFile, string URI, string username, string password)
        {
            FtpWebRequest req = (FtpWebRequest)WebRequest.Create(URI + localFile);
            req.Credentials = new NetworkCredential(username, password);
        ...
        {
            return GetFileList("ftp://192.168.1.160/" + path, WebRequestMethods.Ftp.ListDirectoryDetails);
        }
        //都调用这个
        //上面的代码示例了如何从ftp服务器上获得文件列表
        private static string[] GetFileList(string path, string WRMethods)
        {
            StringBuilder result = new StringBuilder();
            try
            {
                FtpWebRequest reqFTP = GetRequest(path, "powertms4.5", "123");//建立FTP连接
        ...
        public static void MakeDir(string dirName)
        {
            try
            {
                string uri = "ftp://192.168.1.160/" + dirName;
                FtpWebRequest reqFTP = GetRequest(uri, "powertms4.5", "123");//建立FTP连接      
